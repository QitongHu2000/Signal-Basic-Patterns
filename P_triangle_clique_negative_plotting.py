...
times = load_dict('P_triangle_clique_P_'+str(int(100*a))+'_'+str(int(100*b))).tolist()[0]
weights = load_dict('P_triangle_clique_P_weight_'+str(int(100*a))+'_'+str(int(100*b)))
...
times= np.power(weights * 1.0, 1.0/a) * (np.power(weights[0] * 1.0, 1.0/a-1.0)/np.power(weights[0] * 1.0, 1.0/a))#np.array((final_times[:,2]).T.tolist()[0])/1000
ax.scatter(weights,times,s=200,c= colors[2],label = r'$d_i^{\theta_J+1}$')
...
plt.tight_layout()
plt.savefig('P_triangle_clique_P_'+str(int(100*a))+'_'+str(int(100*b))+'.pdf',dpi=300)
plt.show()