...
def simulation(A):
    def F(A,x,B,C):
        #B=0.01
        #alpha = 0.01
        return np.mat(-B*np.multiply(x,np.power(1-x/C,2))+alpha*np.multiply(x,A*x))
...
save_dict(times_simulation_edges, 'I_triangle_clique_I_'+str(int(100*B))+'_'+str(int(100*C)))
save_dict(weights, 'I_triangle_clique_I_weight_'+str(int(100*B))+'_'+str(int(100*C)))
